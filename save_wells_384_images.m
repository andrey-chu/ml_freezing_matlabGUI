...
%            [freeze_num(j,k), freeze_temp(j,k)]=pupi_run(squeeze(wells(:,:,:,j,k)), temperature_array, j, k);
            well_stack = wells(:,:,:,j,k);
            features_vec = zeros(fignum,14);
            for l = 1:fignum
                glcm = graycomatrix(well_stack(:,:,l), 'offset', [0 1], 'Symmetric', true);
                features_vec(l,:) = haralickTextureFeatures(glcm);
            end
            save(strcat(directory,'/wells/well_',num2str(j),'_',num2str(k),'.mat'), 'well_stack', 'features_vec');
