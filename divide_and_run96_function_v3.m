function divide_and_run96_function_v3(directory, tempfile)
cwd = pwd;
...
for j = 1:length(centers_x)
    for i = 1:length(figlist)
        ...
        for k = 1:length(centers_y)
            ...
            wells(:,:,i,k) = fig(centers_y(k)-106/2+1:centers_y(k)+106/2,centers_x(j)-106/2+1:centers_x(j)+106/2, 1);
...
%             fprintf('Column %i out of %i: %4.2f%%\n', k, length(centers_y), (k/length(centers_y))*100);
            ...
            [freeze_num(j,k), freeze_temp(j,k)]=pupi_run(squeeze(wells(:,:,:,k)), temperature_array, j, k);
