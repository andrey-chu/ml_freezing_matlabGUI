...
for j = 1:length(centers_x)
   for k = 1:length(centers_y)
        for i = 1:length(figlist)
            ...
            wells_temp(:,:,i) = fig(centers_y(k)-106/2+1:centers_y(k)+106/2,centers_x(j)-106/2+1:centers_x(j)+106/2, 1);
        ...
        [freeze_num(j,k), freeze_temp(j,k)]=pupi_run(squeeze(wells_temp), temperature_array, j, k);
